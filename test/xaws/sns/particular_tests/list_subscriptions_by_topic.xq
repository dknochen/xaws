...
(:~
 : <p>
 :      This test returns a (limited) list of the topics subscriptions.
 :
 :      NOTE: The result list is limited by AWS. If there are more topics, 
 :      a next-token is also returned and can be used in a new call of 
 :      ListTopics to get further results.
 :      This test will only return the first part of the list.
 : </p>
 :
 : @author Klaus Wichmann klaus [at] xquery [dot] co [dot] uk
 :)
module namespace test = 'http://test/xaws/sns/particular_tests/list_subscriptions_by_topic';
...
declare %ann:sequential function test:run($testconfig as element(config),$testresult as element(testresult)) as element(testresult) {
    ...
    variable $msg := ();
    variable $testname := "sns_list_subscriptions_by_topic";
    ...
    try {
        ...
        let $result := topic:list-subscriptions-by-topic($aws-key,$aws-secret,$topic-arn)[2]
        
        return 
            (: save the list in the testresult-message :)
            $msg := $result//aws:member;
            $success := true();
